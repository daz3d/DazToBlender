...
def rotate(kakudo,xyz):
    ...
    if BV<2.80:
        ...
        t = [False,False,False]
        t[index] = True
        bpy.ops.transform.rotate(value=math.radians(kakudo), orient_axis=xyz,constraint_axis=(t[0],t[1],t[2]))
...
def foot_ikbone_rotate(i):
    if BV<2.80:
        bpy.ops.transform.rotate(value=math.radians(17 + (i * -34)),
                                  axis = (0,1,0),
                                 )
        bpy.ops.transform.rotate(value=math.radians(-27),
                                  axis=(1, 0, 0),
                                 )
    elif BV<2.83:
        bpy.ops.transform.rotate(value=math.radians(-17 + (i * 34)), orient_axis='Y',
...
def import_obj(path):
    ...
    if BV<2.80:
        bpy.ops.import_scene.obj(filepath=path, axis_forward='-Z', axis_up='Y', filter_glob="*.obj;*.mtl",
                                 use_smooth_groups=True, use_split_objects=True,
                                 use_split_groups=True, use_groups_as_vgroups=False, use_image_search=True,
                                 split_mode='OFF', global_clamp_size=0.0)
    else:
        bpy.ops.import_scene.obj(filepath=path, axis_forward='-Z', axis_up='Y', filter_glob="*.obj;*.mtl",
                                 use_smooth_groups=True, use_split_objects=True,
                                 use_split_groups=True, use_groups_as_vgroups=False, use_image_search=True,
