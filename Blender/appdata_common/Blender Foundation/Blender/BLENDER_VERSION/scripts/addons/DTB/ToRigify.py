...
class ToRigify:
                                ...
                                for torigify in db.toRigify:
                                    if torigify[0] >= 2:
                                        ...
                                        if mparent == ops_torigify:
                                            nparent = 'DEF-' + torigify[2].replace('.L', '.R')
                                        elif mparent == torigify[1]:
                                            nparent = 'DEF-' + torigify[2]
                                        if nparent != '' and nparent in robj.data.edit_bones:
                                            eb.parent = robj.data.edit_bones.get(nparent)
                                            self.to_layer(eb, 3)
                                            find = True
                                            break
                    if find==False:
    ...
    def finish_job(self):
        ...
        # if nper in rig.pose.bones:
        #     rig.pose.bones.get(nper).scale[0] = 0.9
            ...
            for b in amtr.data.edit_bones:
                ...
                for tfc in db.tometaface_couple:
                    if z==0 and bname==tfc[1]:
                        clist.append([b.name, [b.tail[0],b.tail[1],b.tail[2]] , [b.head[0],b.head[1],b.head[2] ]]  )
    ...
    def getPlainRol(self,db,plainbone):
        for r in DataBase.tbl_brollfix:
            if r[0] == plainbone or (r[0].startswith("-") and r[0][1:]==plainbone[1:]):
                roll = r[1]
    ...
    def all_rigity_bone(self,db):
        ...
        Global.setOpsMode('EDIT')
        b5 = [['thigh','Thigh','ThighBend'],['shin','Shin','Shin'],['upper_arm','Shldr','ShldrBend'],['forearm','Forearm','ForearmBend'],['foot','Foot','Foot'],['hand','Hand','Hand']]
        lr2 = ['L','R']
        find = False
        miss = ""
        get = ""
        ...
        for pb in rig.pose.bones:
            find = False
            mix = [0,0,0,0,0,0]
            if (('fk.' in pb.name) and pb.name.startswith('MCH-')==False and \
                    ('hand' in pb.name)==False and ('upper_arm.' in pb.name)==False)\
                    or pb.name[0:len(pb.name)-1]=='thumb.01.' or \
                    pb.name[0:len(pb.name)-1]=='toe.':
                ...
                for tbl in db.toRigify:
                    ...
                    if pb.name==tbl[2] or bool_ops:
                        for gbl in Global.get_bone_limit():
                            if (gbl[0]==tbl[1] and bool_ops==False) or (bool_ops and gbl[0]==ops_tbl[1]):
                                line = ""
                                for i in range(6):
                                    mix[i] = gbl[2+i]
            ...
            if find == True:
                ...
                if 'toe' in pb.name:
                    for i in range(3):
                        ...
                        mix[i * 2] = 0 - mix[i * 2 + 1]
                        mix[i * 2 + 1] = 0 - wk
                skips = ['foot_ik.L','foot_ik.R','hand_ik.L','hand_ik.R','ORG-hand.L','ORG-hand.R']
                skip_ik = False
                for skip in skips:
                    if pb.name==skip:
                        skip_ik = True
    ...
    def adjust_pose_bones(self,pb):
        if pb.name.startswith("f_") and (pb.name.endswith(".L") or pb.name.endswith(".R")):
        ...
        if pb.name=='DEF-shin.L' or pb.name == 'DEF-shin.R':
            for c in pb.constraints:
                if c.name == 'Copy Transforms':
                    c.mute = True
        back_reverses = [['upper_arm_tweak','upper_arm_fk'],
                        ...
                        (bool_ops and  dmr[1].startswith('l') and b8[0] == 'r' + dmr[1][1:] ):
                            ...
                            for i in range(3):
                                if dmr[0]!=4:
                                    ...
                                    if sw==0:
                                        meb.roll = b8[7]
                elif keep_key.startswith('f_') and (keep_key.endswith('.R') or keep_key.endswith('.L')):
                    if (keep_key == meb.name or bool_keep_ops):
                        for b8 in blist:
                            if (bool_keep_ops == False and b8[0] == dmr[1]) or \
                                    (bool_keep_ops and dmr[1].startswith('l') and b8[0] == 'r' + dmr[1][1:]):
                                meb.roll = b8[7]
        ...
        for vname in dobj.vertex_groups.keys():
            for dbone in db.toRigify:
                ...
                if dbone[1]==vname or bool_ops:
                    if bool_ops:
                        dobj.vertex_groups[vname].name = ('DEF-' + dbone[2].replace('.L','.R'))
                    else:
